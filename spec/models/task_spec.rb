...
RSpec.describe Task, type: :model do
  ...
  describe "accessible fields" do
    ...
    it { is_expected.to allow_value("").for(  :due                  ) }
...
=end  
  describe "valid tasks" do
    let(:user)      { FactoryGirl.create(:user)               }
    let(:task)      { FactoryGirl.create(:task, user: user)   }
    let(:due_date)  { (Date.today + 4)                        }
    
    it "Saves a valid tasks" do
      expect(task.title).to         eq( "First task"          )
      expect(task.description).to   eq( "First description"   )
      expect(task.due.year).to      eq( due_date.year         )
      expect(task.due.month).to     eq( due_date.month        )
      expect(task.due.day).to       eq( due_date.day          )
